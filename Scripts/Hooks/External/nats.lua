...
function network.write(client, buffer)
    ...
    if client.parameters.send_timeout then
        client.network.socket:settimeout(client.parameters.send_timeout)
    end
...
function network.read(client, len)
    ...
    if client.parameters.receive_timeout then
        client.network.socket:settimeout(client.parameters.receive_timeout)
    end
...
client_prototype.upgrade_to_tls = function(client)
    ...
    client.network.socket, err = luasec.wrap(client.network.socket, params)
    if not client.network.socket then
        nats.error('failed to wrap socket with TLS: ' .. tostring(err))
    end
    -- set SNI
    client.network.socket:sni(client.parameters.host)
    -- Perform TLS handshake
    local status, err = client.network.socket:dohandshake()
    if not status then
        nats.error('failed to perform TLS handshake: ' .. tostring(err))
    end
    -- -- change timeout to receive_timeout after TLS handshake
    -- client.network.socket:settimeout(client.parameters.receive_timeout)
end
...
local function connect_tcp(socket, parameters)
    ...
    if parameters.timeout then
        socket:settimeout(parameters.timeout)
    end
...
function nats.connect(...)
    ...
    elseif #args > 1 then
        ...
        parameters = { host = host, port = port, timeout = tonumber(timeout), receive_timeout = tonumber(timeout), send_timeout = tonumber(timeout) }
    end
